...
geom_line() 
# on odds scale
ggplot(pred_data, aes(x = x, y = yhat_odds, group = f,
                      color = f)) + 
geom_line() 
...
# for the interaction: one additional biosperic value increases the interaction
# effect on the odds
...
LogOdds_Prob(mod$coefficients %*% c(1,1,1,1))/
  (1-LogOdds_Prob(mod$coefficients %*% c(1,1,1,1))) /(
    LogOdds_Prob(mod$coefficients %*% c(1,0,1,0))/
...
beta_range <- seq(0.55, 0.70, length = 20)
...
results <- get_power(data = da_intA,
                     cond ="KGA_IntA",
                     cov = "MW_Bio",
...
# save results
write.table(results, "results/logistic_power_inta_ccc.dat", row.names = FALSE, col.names = FALSE)
...
# save results
write.table(results, "results/logistic_power_inta_ccp.dat", row.names = FALSE, col.names = FALSE)
...
# get power for range of betas
results <- get_power(data = da_intB,
                     cond ="KGB_IntB",
                     cov = "MW_Bio",
                     intercept = mod$coefficients["(Intercept)"],
...
# save results
write.table(results, "results/logistic_power_intb_ccc.dat", row.names = FALSE, col.names = FALSE)
...
# save results
write.table(results, "results/logistic_power_intb_ccp.dat", row.names = FALSE, col.names = FALSE)

# summarise results -------------------------------------------------------
results_inta_ccc <- read.table("results/logistic_power_inta_ccc.dat")$V1
results_inta_ccp <- read.table("results/logistic_power_inta_ccp.dat")$V1
results_intb_ccc <- read.table("results/logistic_power_intb_ccc.dat")$V1
results_intb_ccp <- read.table("results/logistic_power_intb_ccp.dat")$V1
...
# show odds ratio that corresponds (nearly) to a power of 0.8
plotdat_inta_ccc %>% 
