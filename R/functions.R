...
get_power_logistic <- function(data, cond,  covariate, intercept, beta_cond, beta_cov, beta_int){
...
get_power_lm <- function(data, cond,  covariate, intercept, beta_cond,
                         beta_cov, beta_int, res_std_error){
  length_list = length(beta_int)
  pmap_dbl(list(df = rep(list(data),length_list),
                cond = rep(list(cond),length_list), 
                covariate = rep(list(covariate),length_list),
           ...
           .f = function(df, cond, covariate, intercept, beta_cond, beta_cov, beta_int,res_std_error){
             # run mc replications
             tmp_results <- replicate(n = mc, expr = {
               cond = df[[cond]]
               covariate = df[[covariate]]
